...
/**
 * Todo tu código original vive acá.
 * Este subcomponente SÍ usa useSearchParams y está cubierto por <Suspense>.
 */
function MarketplaceContent() {
  ...
  const [cart, setCart] = useState([]); // { id, title, unitPrice, quantity }
  ...
  async function checkout() {
    try {
      ...
      const payload = { items: cart.map((c) => ({ productId: c.id, quantity: c.quantity })) };
      ...
      setCart([]);
      alert(`Compra OK. Tickets emitidos: ${data.ticketsIssued}. ¡Ya podés usarlos en los sorteos!`);
    } catch (e) {
